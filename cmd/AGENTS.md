...
- **daemon.go**: poll loop; `runDaemonCycle` times and classifies each per-cycle error, recording the outcome into Prometheus via the injected `*metrics.Metrics` (nil when disabled → nil-safe no-op). When `metrics.enabled`, `startMetrics` opens a dedicated store, registers the `internal/metrics` library collector, and serves `/metrics` on `metrics.address` (default `:9101`) for the daemon's lifetime, shutting it down on exit.
