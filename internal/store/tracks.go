...
func (s *Store) MarkTrack(ctx context.Context, tr Track) error {
	const q = `INSERT INTO tracks
		(tidal_id, isrc, album_id, path, obtained_quality, status, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, unixepoch())
	...
	if _, err := s.db.ExecContext(ctx, q,
		tr.TidalID, tr.ISRC, tr.AlbumID, tr.Path, tr.ObtainedQuality, string(tr.Status),
	); err != nil {
...
func (s *Store) GetTrack(ctx context.Context, tidalID string) (Track, error) {
	const q = `SELECT tidal_id, isrc, album_id, path, obtained_quality, status, updated_at
	...
	err := s.db.QueryRowContext(ctx, q, tidalID).Scan(
		...
		&tr.ObtainedQuality, &status, &tr.UpdatedAt,
	)
	switch {
